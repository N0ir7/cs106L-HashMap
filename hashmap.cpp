...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::iterator HashMap<K, M, H>::make_iterator(const node* curr) {
    if (curr == nullptr) {
...
HashMap<K, M, H>::HashMap(const InputIt first,const InputIt last, size_t bucket_count, const H& hash) :
...
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const std::initializer_list<value_type> init, size_t bucket_count, const H& hash) :
